...
public class PfType {
	@SuppressLint("NewApi") 
	public static void getPfType(final Pjzbx obj, LinearLayout selected, final Context context, boolean type){
		if(obj.getPfbzList().size() == 0){//输入分数
			...
			editview.setPadding(StringUtils.dip2px(context, 10),StringUtils.dip2px(context, 10),StringUtils.dip2px(context, 10),StringUtils.dip2px(context, 10));
				...
				public void onFocusChange(View v, boolean hasFocus) {
					...
					if(!hasFocus){
						...
						if(e.getText().toString().endsWith("分")){
							e.setText(e.getText().toString().substring(0, e.getText().toString().length()-1));
						}
					}
				}
			});
			...
			if(!type){
				...
				String showtext = (obj.getPjdf() == null || obj.getPjdf().equals(""))?"":(obj.getPjdf().intValue())+"分";
